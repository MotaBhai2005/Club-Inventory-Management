        ...
        {hasRequests ? (
          ...
          <div className="p-8 text-center">
            <div className="mx-auto max-w-md">
              <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100">No requests yet</h3>
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">Submit a request and it will appear here with status updates.</p>
              <button
                onClick={isAdmin ? loadRequests : openNewRequestModal}
                className="mt-5 inline-flex items-center justify-center px-4 py-2 rounded-lg text-sm font-medium bg-brand-600 hover:bg-brand-700 text-white shadow-sm transition-colors"
              >
                {isAdmin ? "Refresh Requests" : "New Request"}
              </button>
            </div>
          </div>
        )}
